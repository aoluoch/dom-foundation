...
import { getEvents, getInvolvedOptions, getPageCopy } from '../lib/contentful'
...
async function loadGetInvolved() {
  const [page, eventsPage, options, events] = await Promise.all([
    getPageCopy('get-involved'),
    getPageCopy('get-involved-events'),
    getInvolvedOptions(),
    getEvents(),
  ])
  return { page, eventsPage, options, events }
}

export default function GetInvolved() {
  const { data, loading, error } = useAsync(loadGetInvolved, [])

  if (error) return <ErrorState />
  if (loading || !data) return <Loader />
  if (!data.page?.title) return <ErrorState message="Get Involved page copy is missing in Contentful." />
  ...
  return (
    <>
      ...
      {data.eventsPage?.title && (
        <section id="events" className="scroll-mt-24 bg-white py-20">
          <div className="container-page">
            <SectionHeading eyebrow={data.eventsPage.eyebrow} title={data.eventsPage.title} align="center" />
